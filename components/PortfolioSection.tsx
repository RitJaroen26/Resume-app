...
export default function PortfolioSection() {
    ...
    const [selectedMiniProject, setSelectedMiniProject] = useState<Project | null>(null);
    const [isVisible, setIsVisible] = useState(false);
    const [isMobile, setIsMobile] = useState(false);
    const containerRef = useRef(null);
    const containerMiniRef = useRef(null);
    const [selectedCategory, setSelectedCategory] = useState<string>('All');
    const [selectedCategoryMini, setSelectedCategoryMini] = useState<string>('All');
    const [isComboboxOpen, setIsComboboxOpen] = useState(false);
    const comboboxRef = useRef<HTMLDivElement>(null);
    const [isComboboxMiniOpen, setIsComboboxMiniOpen] = useState(false);
    const comboboxMiniRef = useRef<HTMLDivElement>(null);
    ...
    useEffect(() => {
        if (!isComboboxMiniOpen) return;
        const handleClickMiniOutside = (event: MouseEvent) => {
            if (comboboxMiniRef.current && !comboboxMiniRef.current.contains(event.target as Node)) {
                setIsComboboxMiniOpen(false);
            }
        };

        if (isComboboxMiniOpen) {
    ...
    useEffect(() => {
        setCurrentIndexMini(0);
    }, [selectedCategoryMini]);
    ...
    const nextSlideMiniProject = () => {
        setCurrentIndexMini((prev) => (prev + 1) % filteredMiniProjects.length);
    };
    ...
    const getVisibleMiniProjects = () => {
        ...
        if (isMobile) {
            ...
            for (let i = 0; i < Math.min(3, filteredMiniProjects.length); i++) {
                const index = (currentIndexMini + i) % filteredMiniProjects.length;
                result.push({ ...filteredMiniProjects[index], position: i });
            }
            return result;
        }
    };
    ...
    const handleCategoryMiniSelect = (categoriyMiniValue: string) => {
        setSelectedCategoryMini(categoriyMiniValue);
        setIsComboboxMiniOpen(false);
        setCurrentIndexMini(0);
    }
            ...
            <div className="relative z-10 container mx-auto px-6">
                ...
                >
                    ...
                    <div className='flex flex-col-reverse md:flex-row justify-between md:ml-10 items-center mt-20'>
                        <h1 className={`text-[20px] md:text-[30px] ${isDarkMode ? "text-white" : "text-gray-600"} mb-4 text-[30px] font-semibold`}>Projects</h1>
            ...
            <div className="relative z-10 container mx-auto px-6">
                ...
                >
                    <div className='flex flex-col-reverse md:flex-row justify-between md:ml-10 items-center mt-20'>
                        ...
                        <div className="flex justify-center mb-6">
                            <div className="relative" ref={comboboxMiniRef}>
                                <motion.button
                                    whileTap={{ scale: 0.98 }}
                                    onClick={() => setIsComboboxMiniOpen(!isComboboxMiniOpen)}
                                    className={`
                                    flex items-center gap-3 px-6 py-3 rounded-xl font-medium text-sm
                                    ${isDarkMode
                                            ? 'bg-gray-800/80 backdrop-blur-sm border border-gray-700 text-white hover:bg-gray-700/80'
                                    ...
                                    shadow-lg hover:shadow-xl transition-all duration-300 min-w-[200px] justify-between
                                    ${isComboboxMiniOpen ? 'ring-2 ring-blue-500/50' : ''}
                                `}
                                >
                                    <div className="flex items-center gap-2">
                                        {selectedCategoryMiniData.icon}
                                        <span>{selectedCategoryMiniData.label}</span>
                                    </div>
                                    <ChevronDown
                                        size={16}
                                        className={`transition-transform duration-200 ${isComboboxMiniOpen ? 'rotate-180' : ''}`}
                                    />
                                </motion.button>
                                ...
                                <AnimatePresence>
                                    {isComboboxMiniOpen && (
                                        ...
                                        `}
                                        >
                                            {categoriesMini.map((category, index) => (
                                                <motion.button
                                                    key={category.value}
                                                    initial={{ opacity: 0, x: -10 }}
                                                    animate={{ opacity: 1, x: 0 }}
                                                    transition={{ delay: index * 0.05 }}
                                                    onClick={() => handleCategoryMiniSelect(category.value)}
                                                    className={`
                                                    w-full flex items-center gap-3 px-4 py-3 text-left text-sm
                                                    ${isDarkMode
                                                            ? 'hover:bg-gray-700/50 text-gray-300'
                                                    ...
                                                    transition-all duration-200 group relative
                                                    ${selectedCategoryMini === category.value
                                                            ? (isDarkMode ? 'bg-blue-600/20' : 'bg-blue-50')
                                                            : ''
                                                        }
                                                `}
                                                >
                                                    <div className="flex items-center gap-2 flex-1">
                                                        <span className={`${selectedCategoryMini === category.value ? 'text-blue-500' : ''}`}>
                                                            {category.icon}
                                                        </span>
                                                        <span className={`${selectedCategoryMini === category.value ? 'font-medium' : ''}`}>
                                                            {category.label}
                                                        </span>
                                                    </div>
                                                    {selectedCategoryMini === category.value && (
                ...
                {filteredMiniProjects.length > 0 ? (
                    <motion.div
                        ref={containerMiniRef}
                        initial={{ opacity: 0, y: 50 }}
                        animate={{ opacity: isVisible ? 1 : 0, y: isVisible ? 0 : 50 }}
                        transition={{ duration: 0.6, delay: 0.2 }}
                        className="relative"
                        key={selectedCategoryMini}
                    >
                        {filteredMiniProjects.length > 1 && (
                            <>
                                <button
                                    onClick={prevSlideMiniProject}
                                    className={`absolute left-2 md:left-4 top-1/2 transform -translate-y-1/2 z-20 p-2 md:p-3 rounded-full ${isDarkMode ? 'bg-gray-800 hover:bg-gray-700 text-white' : 'bg-white hover:bg-gray-50 text-gray-800'} shadow-lg transition-all duration-300 hover:scale-110 cursor-pointer`}
                                >
                                    <ChevronLeft size={isMobile ? 20 : 24} />
                                </button>
                        ...
                        <div className="flex justify-center items-center min-h-[550px] md:min-h-[600px] px-12 md:px-16">
                            <div className={`${isMobile ? 'w-full max-w-sm' : 'flex gap-6 w-full max-w-6xl'}`}>
                                {getVisibleMiniProjects().map((project, index) => (
                                    <motion.div
                                        key={`${project.id}-${currentIndexMini}`}
                                        initial={{
                                            opacity: 0,
                                            scale: 0.95,
                                            y: isMobile ? 20 : 0,
                                    ...
                                    >
                                        <div className={`${isDarkMode ? 'bg-gray-800/50' : 'bg-white'} backdrop-blur-sm rounded-2xl overflow-hidden shadow-xl ${isDarkMode ? 'border-gray-700' : 'border-gray-100'} transition-all duration-500 ease-out hover:shadow-2xl hover:scale-[1.02] group flex flex-col h-full min-h-[500px]`}>
                                            ...
                                            <div className="p-6 flex flex-col flex-1">
                                                ...
                                                <div className="flex gap-3 mt-auto">
                                                    <button
                                                        onClick={() => openMiniProjectDetails(project)}
                                                        className="flex-1 cursor-pointer bg-gradient-to-r from-blue-600 to-purple-600 text-white py-2 px-4 rounded-lg font-medium transition-all duration-300 ease-out hover:shadow-lg hover:shadow-blue-500/25 hover:scale-[1.02] flex items-center justify-center gap-2 group-hover:from-blue-700 group-hover:to-purple-700"
                                                    >
                                                        <Eye size={16} className="transition-transform duration-200 group-hover:scale-110" />
                                                        Details
                        ...
                        {filteredMiniProjects.length > 1 && (
                            <div className="flex justify-center gap-2 mt-8">
                                {filteredMiniProjects.map((_, index) => (
                                    <button
                                        key={index}
                                        onClick={() => setCurrentIndexMini(index)}
                                        className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentIndexMini
                                            ? 'bg-gradient-to-r from-blue-600 to-purple-600 scale-125'
                                            : isDarkMode
                                                ? 'bg-gray-600 hover:bg-gray-500'
                                                : 'bg-gray-300 hover:bg-gray-400'
            ...
            <AnimatePresence>
                {selectedMiniProject && (
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
                        onClick={closeMiniProjectDetails}
                    >
                        <motion.div
                            initial={{ scale: 0.8, opacity: 0 }}
                            animate={{ scale: 1, opacity: 1 }}
